...
반드시 현재 시장 상황(2026년 기준)과 각 종목의 최신 추세를 반영하세요.
매번 새로운 분석을 제공하세요 — 기계적 반복 금지.

제약:
- 보유수량(qty)이 1주(또는 코인의 경우 분할매도가 의미 없는 소액)인 종목에는 "부분매도", "비중축소", "분할매수"를 사용하지 마세요. 대신 "보유유지", "매수", "매도권장", "손절매도" 중에서 선택하세요.
- 현금_비율_퍼센트가 **15 이상이면** 즉시 투입 가능한 유휴자금이 충분하다는 신호로 간주하세요. 이 경우:
  (1) 펀더멘털 점수 60점 이상이거나 RSI 35 이하의 과매도 구간인 **보유종목**에 우선적으로 "분할매수" 또는 "적극매수"를 권장 (신규 종목보다 보유종목 추매가 1순위).
  (2) rebalancing.actions에 구체적인 추매 대상과 금액을 **high 우선순위**로 명시. 예: {"priority":"high","type":"매수","target":"리노공업(058470)","description":"한투 예수금 270만원 중 150만원 분할매수 — 펀더멘털 73점, 반도체 부품 강세"}.
  (3) 어느 계좌(한투/업비트/ISA)의 현금을 사용할지 명시.
- 현금 비율이 5% 미만이면 신규 매수 자제하고 비중조정/현금확보를 우선 권장.
- 추매 권고는 한 줄짜리 일반 조언이 아닌, 종목·금액·계좌가 명시된 실행 가능한 형태여야 함."""
...
async def analyze_portfolio(req: AnalyzeRequest):
    ...
    # 현금 합계(한투+업비트+ISA) 및 비율 계산 — 프롬프트가 추매 추천 판단에 사용
    cash_dict = req.cash or {}
    isa_cash = float(((req.isa or {}).get("cash") or 0))
    ...
    total_asset = float(req.totalAsset or 0)
